...
async function loadJson(path){
  const res = await fetch(path + '?v=15', {cache:'no-store'});
...
async function init(){
  try{
    const [eventData, logeData] = await Promise.all([
      loadJson('events.json'),
      loadJson('logeaftener.json')
    ]);
    events = Array.isArray(eventData) ? eventData : [];
    logeaftener = Array.isArray(logeData) ? logeData : [];
  }catch(err){
    console.error('Kunne ikke indlæse JSON-filer:', err);
    nextEvent.innerHTML = `<div class="empty">Kunne ikke indlæse events.json.</div>`;
    nextLoge.innerHTML = `<div class="empty">Kunne ikke indlæse logeaftener.json.</div>`;
    return;
  }
  renderAll();
}
...
$$('.nav-btn').forEach(btn => btn.addEventListener('click', () => {
  ...
  $('#view' + v[0].toUpperCase() + v.slice(1)).classList.add('active-view');
