...
/* V8: Fjern separat "Se plakat"-flow. Kort -> detaljer. Plakat i detaljer -> zoom. */
(() => {
  function cleanPosterButtons() {
    const selectors = [
      "[data-show-poster]",
      "[data-poster]",
      "[data-poster-src]",
    ...
    document.querySelectorAll(selectors.join(",")).forEach((el) => {
      ...
      if (
        txt.includes("plakat") ||
        el.matches("[data-show-poster], [data-poster], [data-poster-src], .poster-button, .poster-link, .view-poster, .show-poster, .se-poster")
      ) {
    ...
    document.querySelectorAll("button, a").forEach((el) => {
      ...
      if (txt === "se plakat" || txt === "plakat" || txt.includes("zoom plakat")) {
        el.style.display = "none";
        el.setAttribute("aria-hidden", "true");
      }
    });
  ...
  cleanPosterButtons();
  const observer = new MutationObserver(cleanPosterButtons);
  observer.observe(document.documentElement, { childList: true, subtree: true });

  // Når en plakat vises i detaljevisningen, kan man trykke direkte på den for fuld zoom.
  document.addEventListener("click", (e) => {
    const img = e.target.closest("img");
    if (!img) return;

    const looksLikePoster =
      img.classList.contains("hero-poster") ||
      img.classList.contains("detail-poster") ||
      img.classList.contains("poster-preview") ||
      img.closest(".event-detail, .modal-card, dialog, .detail-view");

    if (looksLikePoster && typeof window.openPosterZoom === "function") {
      e.preventDefault();
      e.stopPropagation();
      window.openPosterZoom(img.currentSrc || img.src, img.alt || "Plakat");
    }
  }, true);
})();
